...
public class CrontabEntryBean implements Serializable {
	...
	//以友好格式返回
//	public String toWellContent() {
//		try {
//			CrontabParser cp = new CrontabParser();
//			return cp.format(this);
//		} catch (Exception e) {
//			return e.toString();
//		}
//
//	}
	...
	//以友好格式返回
//	public String getWellContent() {
//		return toWellContent();
//	}
	...
	private boolean equalsCalendar(Calendar cal) {
		...
		//System.out.println("equalsCalendar():" + cal.get(Calendar.SECOND) );
		if(!"*".equals(this.getYear()) && !this.getYear().equals(Integer.toString(cal.get(Calendar.YEAR))) ){
			return false;
		}
		return (bSeconds[cal.get(Calendar.SECOND)] && bHours[cal.get(Calendar.HOUR_OF_DAY)]
				&& bMinutes[cal.get(Calendar.MINUTE)] && bMonths[cal.get(Calendar.MONTH)]
				&& bDaysOfWeek[cal.get(Calendar.DAY_OF_WEEK) - 1] && bDaysOfMonth[cal.get(Calendar.DAY_OF_MONTH) - 1])
				&& bYears[0];
	}
	...
	private boolean equalCrontabEntryBean(CrontabEntryBean ceb) {
		...
		if (this.getBExtraInfo()) {
			...
			for (int i = 0; i < this.getExtraInfo().length; i++) {
				if (!this.getExtraInfo()[i].trim().equals(ceb.getExtraInfo()[i].trim()))
					return false;
			}
		}
		return true;
	...
	public boolean isStart() {
		return state.toUpperCase().equals(START);
	}
