...
public class SecurityHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public String getParameter(String name) {
        String value = super.getParameter(securityEncode(name));
    ...
    public String[] getParameterValues(String name) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            parameters[i] = securityEncode(parameters[i]);
        }
        return parameters;
    }
    ...
    public Map<String,String[]> getParameterMap() {
        ...
        for (String key : parameters.keySet()) {
            ...
            for (int i = 0; i < values.length; i++) {
                values[i] = securityEncode(values[i]);
            }
            map.put(key, values);
        }
        return map;
    ...
    public String getHeader(String name) {
        String value = super.getHeader(securityEncode(name));
        if (StringUtils.isNotBlank(value)) {
            value = securityEncode(value);
        }
        return value;
    ...
    public static HttpServletRequest getOrgRequest(HttpServletRequest request) {
        if (request instanceof SecurityHttpServletRequestWrapper) {
            return ((SecurityHttpServletRequestWrapper) request).getOrgRequest();
        }
